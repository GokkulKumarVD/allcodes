...
try:
    ...
    case when queueid like '%priority%' then 'Priority'
    ...
    sum(case when AHT_IN_SECS >=0 then AHT_IN_SECS/60 end) as AHT,
    ...
    Sum(case when queue_wait_time_in_secs>=0 then queue_wait_time_in_secs end ) as queue_wait_time,
     ...
     group by 1,2,3,4,12,16""".format(day_2, day_1, day_2, day_1))

    df = pd.DataFrame(raw_fetch)

    df.columns = ['DATE', 'AGENT_EMAIL', 'CENTER', 'role', 'INTERACTIONS', 'OVER_ALL_FRTINSEC', 'FRT_COUNT',
                  'PROMOTORS', 'ALL_CSAT', 'AHT', 'QUEUE_WAIT_TIME', 'READY_HOURS', 'FTNR', 'TOTAL_ORDERS',
                  'CUNCURRENCY', 'LOGIN_HOURS']

    data = df
    data['OVERALL AHT'] = (data['AHT'] / data['FRT_COUNT'])
    # data['OVERALL AHT'] = (data['AHT'] / data['FRT_COUNT']) / 1440
    data['CSAT'] = (data['PROMOTORS'] / data['ALL_CSAT']) * 100
    data['FTR'] = ((data['TOTAL_ORDERS'] - data['FTNR']) / data['TOTAL_ORDERS']) * 100
    data['FRT'] = (data['OVER_ALL_FRTINSEC'] / data['FRT_COUNT'])
    # data['FRT'] = (data['OVER_ALL_FRTINSEC'] / data['FRT_COUNT']) / 86400
    ...
    # storing the subject
    msg['Subject'] = "Legacry day - 2 susccesful"

    # string to store the body of the mail
    body = "legacy day - 2 updated"
    ...
    # string to store the body of the mail
    body = "There is an error in legacy day - 2 file" + str(err)
