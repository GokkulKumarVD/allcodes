...
if proceed:
        ...
        try:
            ...
            if time_check >= 0 | time_check <= 6:
                ...
                raw_fetch = cs.execute("""select
                                        ...
                                        Sum(CHAT_COUNT)
                                        from analytics.cc_cx.customer_interaction_live
                                        where EVENT_60MIN_SLOT > '{}'  and ((queueid not like '%stores%' and queueid not like '%genie%' and queueid not like '%daily%'
                                        and queueid not in( 'l2-team-14','l2-team-1','l2-team-2','l2-team-10','all-go','daily-fallback',
                                        'cafe-queue','bengali','hindi','telugu','malayalam','kannada','marathi'
                                        )) OR queueid like '%combined%')
                                        group by 1,2;""".format(cy_today))
                print('below 24')
    ...
    threshold_df.to_csv('/var/lakshya/files/threshold.csv', index=False)
