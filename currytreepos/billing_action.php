...
if(isset($_POST["action"]))
{
	...
	if($_POST["action"] == 'fetch_single')
	{
		...
		//discount in order_item code by arbaz
		$object->query = "
		SELECT * FROM order_table 
		WHERE order_id = '".$_POST['order_id']."'";
		$result2 = $object->get_result();
		foreach($result2 as $row2){
			$order_net_amount = $row2["order_net_amount"];
			$order_discount = $row2["discount_percentage"];
			$html .= '
			<tr>
			<td colspan="2" class="text-right"><b>Discount</b></td>
			<td colspan="3"><input type="number" class="form-control discount_percentage" id="dp" data-order_id="'.$row2["order_id"].'" data-order_amount="'.$row2["order_net_amount"].'" value="'.$row2["discount_percentage"].'" /></td>
			<td colspan="1"><input type="button" class="primary" value="apply"/></td>
			</tr>
		';
		}
		...
		echo "hjhjhjh".$order_net_amount;
		echo $order_discount;
		$discount_rate = $order_discount / 100;
		$totaldiscount = $gross_total * $discount_rate;
		$gross_total -= $totaldiscount;
		// $discount = $net_total* 0.04;
		// $net_total -= $discount;
		$order_data = array(
		...
		$html .= '
			<tr>
				<td colspan="4" class="text-right"><b>Net Amount12</b></td>
				<td colspan="2">'.$object->cur . number_format((float)$gross_total, 2, '.', '').'</td>
			</tr>
		';
		$html .= '
	...
	// if($_POST["action"] == 'fetch_discount')
	// {
	// 	$object->query = "
	// 	SELECT * FROM order_item_table 
	// 	WHERE order_id = '".$_POST['order_id']."' 
