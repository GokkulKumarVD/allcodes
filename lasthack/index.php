...
if (isset($_POST['submit'])) {
    ...
    if (preg_match("/{$search}/i", $emailid)) {
        ...
        if (!empty($empid) & !empty($emailid)) {
            $sql = "select * from infoquizweek3 where empid=$empid";
            $result = mysqli_query($mysqli, $sql);
            if (mysqli_num_rows($result) == 0) {

                $sql = "insert into infoquizweek3 (empid, emailid, score, starttime, endtime, timetaken) values('$empid','$emailid','$score','$starttime','$endtime', '$timetaken'); ";
                $result = mysqli_query($mysqli, $sql);
