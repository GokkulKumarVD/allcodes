    ...
    try:
        ...
        try:
            ...
            raw_fetch = cs.execute(
                ...
                "b.AGENTEMAILID != 'NOTFILLED' and to_date(timeadd(hour, -7, assignment_time)) = to_date(current_date) group by 1,2,3,4;")
            ...
            surveys.to_csv("C:\\Users\\"+username+"\\Desktop\\projects\\rnr\\surveys.csv")
            raw_csat = surveys
...
try:
    ...
    less_chat_count = agent_wise[agent_wise['queue'] != 'L2_stores_genie']
    ...
    ed_s_g['local_ranking'] = ed_s_g['score'].rank(ascending=False, method='dense')
    ed_s_g['global_rank'] = ed_s_g['score'].rank(ascending=False, method='dense')

    ed_s_g['csat_global_rank'] = ed_s_g['EFFORTSCORE'].rank(ascending=False, method='dense')
    ed_s_g['frt_global_rank'] = ed_s_g['FRT'].rank(ascending=True, method='dense')
    ed_s_g['aht_global_rank'] = ed_s_g['aht_secs'].rank(ascending=True, method='dense')

    ed_s_g['csat_local_rank'] = ed_s_g['EFFORTSCORE'].rank(ascending=False, method='dense')
    ed_s_g['frt_local_rank'] = ed_s_g['FRT'].rank(ascending=True, method='dense')
    ed_s_g['aht_local_rank'] = ed_s_g['aht_secs'].rank(ascending=True, method='dense')
    ...
    for i in q_list:
        ...
        agent_wise1['global_rank'] = agent_wise1['score'].rank(ascending=False, method='dense')
        agent_wise1.sort_values(by=['global_rank'],inplace = True)

        agent_wise1['csat_global_rank'] = agent_wise1['EFFORTSCORE'].rank(ascending=False, method='dense')
        agent_wise1['frt_global_rank'] = agent_wise1['FRT'].rank(ascending=True, method='dense')
        agent_wise1['aht_global_rank'] = agent_wise1['aht_secs'].rank(ascending=True, method='dense')



        # now finding local ranking
        agent_wise1['local_ranking'] = agent_wise1.groupby('Partners')['score'].rank(ascending=False, method ='dense')

        agent_wise1['csat_local_rank'] = agent_wise1.groupby('Partners')['EFFORTSCORE'].rank(ascending=False, method='dense')
        agent_wise1['frt_local_rank'] = agent_wise1.groupby('Partners')['FRT'].rank(ascending=True, method='dense')
        agent_wise1['aht_local_rank'] = agent_wise1.groupby('Partners')['aht_secs'].rank(ascending=True, method='dense')

        agent_wise_output = pd.DataFrame()


        if i == 'L2_food':
            L2_df = pd.concat([agent_wise1, less_chat_count1])
            df_list.append(L2_df)
        elif i == 'Priority':
            Priority_df = pd.concat([agent_wise1, less_chat_count1])
            df_list.append(Priority_df)
        elif i == 'Stores':
            Stores_df = pd.concat([agent_wise1, less_chat_count1])
            df_list.append(Stores_df)
        elif i == 'Genie':
            Genie_df = pd.concat([agent_wise1, less_chat_count1])
            df_list.append(Genie_df)
        elif i == 'Bau-food':
            bau_food_df = pd.concat([agent_wise1, less_chat_count1])
            df_list.append(bau_food_df)
        elif i == 'L2_stores_genie':
            L2_stores_genie_df = pd.concat([agent_wise1, less_chat_count1, ed_s_g])
            df_list.append(L2_stores_genie_df)

    agent_wise_output = pd.concat(df_list)
    agent_wise_output.fillna(0, inplace=True)

    agent_wise_output.drop_duplicates(subset=['AGENTEMAILID'], keep='first', inplace=True)


    agent_wise_output.to_csv("C:\\Users\\"+username+"\\Desktop\\projects\\rnr\\agent_wise.csv")


    # main df list
    # agent_performance_last_run
    agent_wise_output = agent_wise_output.merge(survey_count, how='left', on='AGENTEMAILID')
    live = agent_wise_output[['AGENTEMAILID','Partners','EFFORTSCORE','FRT','aht_secs','score','csat_local_rank','csat_global_rank','frt_local_rank','frt_global_rank','aht_local_rank','aht_global_rank','local_ranking','global_rank','chat_count','p_n_survey_count','positive_survey_count','negative_survey_count','no_survey','positive_survey_max_streak','FRT_max_streak','aht_max_streak','queue']]
    live.to_csv("C:\\Users\\vd.gokkulkumar\\Desktop\\projects\\rnr\\live.csv")

    # global rank table
    live_global_leader = agent_wise_output[agent_wise_output['global_rank'] == 1]
    live_global_leader = live_global_leader[['AGENTEMAILID','Partners','EFFORTSCORE','FRT','aht_secs','score','queue']]
    live_global_leader.to_csv("C:\\Users\\vd.gokkulkumar\\Desktop\\projects\\rnr\\live_global_leader.csv")


    # local rank leader
    live_local_leader = agent_wise_output[agent_wise_output['local_ranking'] == 1]
    live_local_leader = live_local_leader[['AGENTEMAILID','Partners','EFFORTSCORE','FRT','aht_secs','score','queue']]
    live_local_leader.to_csv("C:\\Users\\vd.gokkulkumar\\Desktop\\projects\\rnr\\live_local_leader.csv")
    ...
    threshold_df.to_csv('C:\\Users\\vd.gokkulkumar\\Desktop\\projects\\rnr\\threshold.csv')
