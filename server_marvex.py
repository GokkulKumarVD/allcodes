...
if proceed:
    ...
    try:
        ...
        if len(igcc_old_check) > 1:
            ...
            food_issue_resolution = pd.DataFrame(raw_fetch)
            food_issue_resolution.columns = ['RESOLUTIONGIVENDATE', 'ISSUEID', 'ORDERID', 'ISSUE', 'ISSUE2',
                                             'RESOLUTIONID',
                                             'AGENT_EMAIL', 'APPROVED_BY_EMAIL', 'IS_RESOLVED', 'RESOLUTION_ID',
                                             'RESOLUTIONS_AMOUNT', 'RESOLUTIONS_GIVEN', 'RESOLUTION_CODES',
                                             'RECOMMENDATIONID',
        ...
        if food_issue_resolution_orderid.__len__() < 9999:
            raw_fetch = cs.execute(
                "Select orderid,status from secure_views.igcc_restaurant_recovery.dp_update_de_order_status_event where orderid in ({}) and dt between current_date - 3 and current_date".format(
            ...
            print(datetime.datetime.now())
            raw_fetch = cs.execute(
                "Select * from secure_views.igcc_restaurant_recovery.dp_place_order where order_id in ('{}') and dt between current_date - 3 and current_date".format(
                    "','".join([str(x) for x in food_issue_resolution_orderid])))
            ...
            raw_fetch = cs.execute(
                "Select orderid,status from secure_views.igcc_restaurant_recovery.dp_update_de_order_status_event where orderid in ({}) and dt between current_date - 3 and current_date".format(
                    ",".join([str(x) for x in food_issue_resolution_orderid[0:10000]])))
            dp_update_de_order_status_event1 = pd.DataFrame(raw_fetch)
            dp_update_de_order_status_event1.columns = ['ORDERID', 'STATUS']

            raw_fetch = cs.execute(
                "Select orderid,status from secure_views.igcc_restaurant_recovery.dp_update_de_order_status_event where orderid in ({}) and dt between current_date - 3 and current_date".format(
                    ",".join([str(x) for x in food_issue_resolution_orderid[10000:]])))
    ...
    try:
        ...
        food_issue_resolution.rename(
            columns={"RECOMMENDATIONID": "ISRECOMMENDATIONFOLLOWED", "ORDERID": "BASE_ORDER_ID",
                     ...
                     "ISSUEDESCL2": "ISSUE2", "ITEMQTYIMPACTED": "ITEMS_QUANTITY_IMPACTED",
    ...
    try:
        ...
        phn1.to_csv("C:\\Users\\vd.gokkulkumar\\Desktop\\projects\\marvex\\phone_numbers.TSV", sep='\t')

        phn = pd.read_csv("C:\\Users\\vd.gokkulkumar\\Desktop\\projects\\marvex\\phone_numbers.TSV", sep='\t', thousands=',')
...
try:
    ...
    no_resto_number = pd.read_csv("C:\\Users\\vd.gokkulkumar\\Desktop\\projects\\marvex\\No_Resto_Number.csv")
    ...
    igcc.to_csv("C:\\Users\\vd.gokkulkumar\\Desktop\\projects\\marvex\\check.csv")
    ...
    igcc = pd.read_csv("C:\\Users\\vd.gokkulkumar\\Desktop\\projects\\marvex\\check.csv")
    igcc['ISSUE2'] = igcc['ISSUE2'].fillna('nothing')
            ...
            return "Midpoint cafe"
                        ...
                        (igcc['REST_CITY_NAME'] == "Hyderabad") or (igcc['REST_CITY_NAME'] == "Gurgaon") or (
                            igcc['REST_CITY_NAME'] == "Kolkata") or (igcc['REST_CITY_NAME'] == "Bangalore") or (
                                    ...
                                    igcc['REST_CITY_NAME'] == "Noida1") or (igcc['REST_CITY_NAME'] == "Coimbatore") or (
                                    igcc['REST_CITY_NAME'] == "Kochi") or (igcc['REST_CITY_NAME'] == "Nagpur")):
            return "Missing items GHK"
        elif (igcc['ISSUE'] == "missing_items") & (igcc['RESTAURANT_ID'] in missing_items_city_list):
            return "Missing items GHK"
